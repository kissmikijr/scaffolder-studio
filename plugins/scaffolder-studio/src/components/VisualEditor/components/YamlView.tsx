...
export const YamlView = ({ templateId, nodes, edges, setNodes: setNodesExternal, setEdges: setEdgesExternal, onAddProperty }: YamlViewProps) => {
  ...
  const [readOnlyRanges, setReadOnlyRanges] = useState<Array<{ from: number; to: number }>>([]);
    ...
    if (id) {
      scaffolderVisualApi
        ...
        .then(async template => {
          ...
          const ranges = calculateReadOnlyRanges(template, nodes, resolvedNodes as Node<AllNodeData>[]);
