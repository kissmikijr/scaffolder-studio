...
export const PrefabList = ({
  ...
  const handleDragStart = (event: React.DragEvent, prefab: Prefab) => {
    ...
    if (prefab.version) {
      event.dataTransfer.setData(
        'application/reactflow/version',
        prefab.version,
      );
    }
    event.dataTransfer.setData(
      'application/reactflow/refType',
      prefab.node.type || '',
    );
        ...
        >
          ...
          >
            <tr>
              ...
              {!compact && <th style={{ textAlign: 'center' }}>Description</th>}
            ...
            {isLoading ? (
              ...
              (() => {
                if (
                  ...
                  (!groups || groups.every(g => g.prefabs.length === 0))
                ) {
                  return (
                    <TableRow>
                      <TableCell
                ...
                return (() => {
                  const groupedByPrefabId = (prefabsToList: Prefab[]) =>
                    Object.values(
                  ...
                  const renderGroup = (group: Prefab[]) => {
                    ...
                    const groupId =
                      latestPrefab.prefabId || latestPrefab.id || 'unknown';
                    const isExpanded = expandedGroups.has(groupId);

                    return (
                      <Fragment key={latestPrefab.id}>
                        <TableRow
                          hover
                          draggable={draggable}
                          onDragStart={e =>
                            draggable && handleDragStart(e, latestPrefab)
                          }
                          onMouseEnter={() =>
                            draggable && handlePrefabHover(latestPrefab)
                          }
                          onDoubleClick={() => {
                            onPrefabClick(
                              latestPrefab.prefabId || '',
                              latestPrefab.version,
                            );
                          }}
                          onContextMenu={e =>
                            handleContextMenu(e, latestPrefab)
                          }
                          sx={{
                        ...
                        >
                          <TableCell align="left" sx={{ minWidth: 160 }}>
                            ...
                            >
                              ...
                              <TemplateVisualSummary
                                nodes={[latestPrefab.node]}
                              />
                            </Box>
                          </TableCell>
                          <TableCell>
                        ...
                        {isExpanded &&
                          sortedGroup.slice(1).map(prefab => (
                            <TableRow
                              ...
                              draggable={draggable}
                              onDragStart={e =>
                                draggable && handleDragStart(e, prefab)
                              }
                              onMouseEnter={() =>
                                draggable && handlePrefabHover(prefab)
                              }
                              onDoubleClick={() =>
                                onPrefabClick(
                                  prefab.prefabId || '',
                          ...
                          <Fragment key={group.title + idx}>
                            {group.prefabs.length > 0 && (
                              <>
                                ...
                                {groupedByPrefabId(group.prefabs).map(
                                  renderGroup,
                                )}
                              </>
                            )}
      ...
      >
        ...
        <DialogContent>
          <DialogContentText id="delete-dialog-description">
            ...
            <strong>{prefabToDelete?.title}</strong> (version{' '}
            {prefabToDelete?.version})?
            <br />
            <br />
            This action cannot be undone. Other versions of this prefab will
