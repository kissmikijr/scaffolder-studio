...
export const PrefabEditor = () => {
  ...
  const getDropdownValueFromNodeType = (type: string): string => {
    switch (type) {
      case 'step':
        return 'step';
      case 'templateOutput':
  ...
  const currentState = useMemo(
    () =>
      prefab
        ...
        : defaultDraftState,
    [prefab, defaultDraftState],
  );
  const { isDirty, syncStatus, lastSyncedAt, saveNow, setPersistedState } =
    usePrefabDraftPersistence({
      prefabId: id,
      state: currentState,
    ...
    api
      ...
      .then(loadedPrefab => {
        ...
        if (prefabToLoad.node) {
          const rehydratedNodes = rehydrateNodes([prefabToLoad.node as any], {
        ...
        setPersistedState({
          node: (loadedPrefab.node ||
            createNodeFromType('step')!) as unknown as Node<AllNodeData>,
  ...
  const handleMetadataChange = useCallback(
    (_: string, data: Partial<StoredPrefab>) => {
      setPrefab(prev =>
        prev ? { ...prev, ...data, is_published: false } : prev,
      );
    },
    [],
  );
          ...
          <Button
            ...
            onClick={async () => {
              ...
              try {
                ...
                alertApi.post({
                  message: `Failed to publish prefab: ${
            ...
            disabled={
              !prefab ||
              (prefab.is_published && !!prefab.published_at) ||
              !canPublishPrefab
            }
            sx={{ height: 32, width: 128 }}
          >
            {(() => {
              if (!prefab?.published_at) return 'Publish';
              return prefab.is_published ? 'Published' : 'Publish';
            })()}
          </Button>
        </PrefabHeader>
        <Box
