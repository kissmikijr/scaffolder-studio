...
const parseDraft = (raw: string | null): StoredPrefabDraft | null => {
  ...
  if (parsed && parsed.version === 1 && parsed.updated && parsed.state?.node) {
    return parsed;
  }
...
export const usePrefabDraftPersistence = ({
  ...
  const serializedState = useMemo(() => toSerializableState(state), [state]);
  ...
  const saveNow = useCallback(
    async (force = true): Promise<boolean> => {
      ...
      setIsSyncing(true);
      const stateToSerialize = currentSerializedStateRef.current;
      const now = new Date().toISOString();

      try {
        await api.update({
          id: prefabId,
          node: stateToSerialize.node,
          title: stateToSerialize.title,
          description: stateToSerialize.description,
        } as any); // Type assertion as api.update might vary slightly

        const stateHash = toStateHash(stateToSerialize);
        setPersistedStateHash(stateHash);
        setHasSyncError(false);
        setLastSyncedAt(now);
    ...
    [prefabId, isSyncing, persistedStateHash, isOnline, api, clearRetryTimeout],
  );
