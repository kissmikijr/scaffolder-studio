...
export const findAllTokens = (text: string): TokenMatch[] => {
    ...
    let match;
    while ((match = PARAM_REGEX.exec(text)) !== null) {
    ...
    while ((match = STEP_REGEX.exec(text)) !== null) {
        ...
        const alreadyCovered = tokens.some(
            t => match!.index >= t.index && match!.index < t.index + t.length
        );
        if (!alreadyCovered) {
            const filterPart = match[3] || '';
            tokens.push({
                index: match.index,
                length: match[0].length,
                fullMatch: match[0],
                content: `steps['${match[1]}'].output['${match[2]}']${filterPart}`,
            });
        }
    }
