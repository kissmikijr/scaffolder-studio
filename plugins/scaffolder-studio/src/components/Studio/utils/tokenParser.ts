...
// Regex patterns for unwrapped tokens with chained filters
// Supports: parameters.foo | lower | trim | replace("a", "b")
const FILTER_CHAIN_REGEX = String.raw`(?:\s*\|\s*[a-zA-Z_][a-zA-Z0-9_]*(?:\([^()]*\))?)*`;
const PARAM_REGEX = new RegExp(
  String.raw`parameters\.([a-zA-Z0-9_]+)(${FILTER_CHAIN_REGEX})`,
  'g',
);
const STEP_REGEX = new RegExp(
  String.raw`steps\['([^']+)'\]\.output\['([^']+)'\](${FILTER_CHAIN_REGEX})`,
  'g',
);
...
export const findAllTokens = (text: string): TokenMatch[] => {
  ...
  while (match !== null) {
    ...
    const alreadyCovered = tokens.some(
      t =>
        currentMatch.index >= t.index &&
        currentMatch.index < t.index + t.length,
    );
    if (!alreadyCovered) {
