...
const ScaffolderStudioEditor = ({
  ...
  const handleAddStepFromToolbar = useCallback(() => {
    ...
    const { width } = getNodeDimensions(source);
    const gap = 90;
  ...
  const handleAddParametersFromToolbar = useCallback(() => {
    ...
    if (isTemplateNode(source)) {
      createParametersNode({
        position: {
          x: source.position.x + width / 2 - 300 / 2, // 300 is default params width
          y: source.position.y + height + gap,
        },
        node: source,
        fromHandleId: 'bottom',
      });
  ...
  const renderShortcutIcon = useCallback(
    (icon: React.ReactNode, shortcut: string) => (
