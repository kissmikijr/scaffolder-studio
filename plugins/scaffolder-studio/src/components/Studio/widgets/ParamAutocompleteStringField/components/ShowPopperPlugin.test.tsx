...
function Wrapper({ children, onEditorReady }: { children: React.ReactNode; onEditorReady?: (editor: LexicalEditor) => void }) {
    return (
...
describe('ShowPopperPlugin', () => {
    ...
    it('replaces the token with a filtered version when a simple filter is applied', async () => {
        ...
        render(
            <Wrapper onEditorReady={e => { editorRef.current = e; }}>
                <ShowPopperPlugin {...defaultProps} showAutocomplete={true} />
            </Wrapper>
        );
