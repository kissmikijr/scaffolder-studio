...
export const parseScaffolderTemplate = (
  ...
  if (yamlData.spec.parameters && yamlData.spec.parameters.length > 0) {
    // Create all parameter nodes first
    const paramNodes = yamlData.spec.parameters.map((param, index) => {
    ...
    yamlData.spec.parameters.forEach((param, paramIndex) => {
  ...
  // Create step nodes
  const stepNodes = yamlData.spec.steps.map((step, index) => {
  ...
  if (yamlData.spec.output) {
    const outputNode = {
      id: uuidv4(),
      type: 'templateOutput',
      position: { x: 0, y: STATIC_Y_POSITION * 3 },
      data: {
        links: yamlData.spec.output.links || [],
        text: yamlData.spec.output.text || [],
      },
    };
