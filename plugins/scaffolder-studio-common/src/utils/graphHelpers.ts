...
import { AllNodeData, PrefabInstanceNodeData } from '../types';
  ...
  while (queue.length > 0) {
    ...
    edges.forEach(edge => {
      if (edge.source === current.id) {
        ...
        if (targetNode && inDegree.get(edge.target) === 0) {
...
export const getDirectChildren = (
  ...
  const directPropertyNodes = nodes.filter(
    n =>
      (childrenIds.includes(n.id) || n.parentId === startNodeId) &&
      n.type === type,
  );
...
export const getOrderedProperties = (
  ...
  const groupProperties = nodes.filter(
    n =>
      ...
      (n.type === 'property' ||
        (n.type === 'prefab' &&
          (n.data as PrefabInstanceNodeData)?.refType === 'property')),
  );
  ...
  while (current) {
    ...
    const outgoingEdge = edges.find(
      e => e.source === currentId && groupNodeIds.has(e.target),
    );
