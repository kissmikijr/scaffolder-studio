...
import { SCAFFOLDER_STUDIO_EVENTS } from '@kissmiklosjr/plugin-scaffolder-studio-common';
...
export class EventPublisherExtension implements PublisherExtension {
  ...
  async publish({ scaffolderTemplate }: PublishContext): Promise<void> {
    await this.events.publish({
      topic: SCAFFOLDER_STUDIO_EVENTS.TEMPLATE_PUBLISHED,
      eventPayload: {
        scaffolderTemplate,
      },
    });
  ...
  async unpublish({ scaffolderTemplate }: UnpublishContext): Promise<void> {
    await this.events.publish({
      topic: SCAFFOLDER_STUDIO_EVENTS.TEMPLATE_UNPUBLISHED,
      eventPayload: {
        scaffolderTemplate,
      },
    });
