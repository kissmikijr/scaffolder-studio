...
test.describe('Scaffolder Visual Editor', () => {
  ...
  test('should create a new template and load the editor', async ({ page }) => {
    ...
    await expect(page).toHaveURL(
      /\/scaffolder-studio\/templates\/.*\/form/,
    );
  ...
  test('should update header title when template name is changed', async ({ page }) => {
    await listPage.goto();
    await listPage.createNewTemplate();
    ...
    await editorPage.editTemplateNode(newName, 'test-owner', 'test description');
